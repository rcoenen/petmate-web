...
CODEX note on direct resident-batch solve (2026-03-11): moving standard solve/refine/finalize to operate directly on resident batch arrays inside the binary WASM kernel removed the last activation-copy hop on the MCM resident path. Targeted `skeletor` `current-defaults` WASM preview capture stayed byte-identical to the pre-change compact path while lowering the isolated capture from `22180.7ms` to `18562.5ms` (`16.3%` faster); `slayer_multi_color.png` remained byte-identical to the current output on the same direct-batch path.
CODEX note on MCM lower-bound pruning (2026-03-11): tightening exact lower bounds in the WASM triple ranker and per-cell pool scorer cut off candidates before the expensive fg/luma/hue scoring when they could not beat the current best. The new path stayed byte-identical to the direct-batch baseline while lowering isolated `current-defaults` preview captures from `18562.5ms` to `14083.7ms` on `skeletor` (`24.1%` faster) and from `21837.0ms` to `13705.2ms` on `slayer_multi_color` (`37.2%` faster).
